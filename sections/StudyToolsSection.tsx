...
const [sel, setSel] = useState<string | null>(null);
  ...
  {/*const boards = cur ? Object.keys(CUR_DATA[cur] || {}) : [];
  const grades = cur && brd ? CUR_DATA[cur]?.[brd] || [] : []; */}
  const tool = TOOLS.find((t) => t.id === sel)!;
 {/* const canGo = cur && brd && grd && top.trim().length > 1; */}
...
>
  ...
  <line x1="80" y1="38" x2="82" y2="44" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" opacity=".6" />
      ...
      >
        ...
        {/* curriculum badges */}
        <Reveal delay={0.18}>
          <div className="mt-5 flex flex-wrap gap-[7px] justify-center">
            {[
              "🇬🇧 UK National",
              "🇬🇧 Cambridge",
              "🇬🇧 Edexcel",
              ...
              <span
                key={b}
                className="px-[13px] py-[5px] border border-border-default rounded-full text-[12px] text-body bg-white cursor-default transition-all duration-150 hover:border-teal hover:text-teal-dk"
              >
                {b}
              </span>
            ))}
